    ...
    <div>
        <div class="row">
            <div class="col-xs-12 mt-1 mb-3">
                <h4>List of Food Supplements 
                 <button  v-if="user.userRole == 'admin'" type="button" class="btn btn-sm btn-primary" style="float:right" @click="dataModalOpen">
                    <i class="icon-plus"></i>ADD
                </button></h4>
                <p>Food supplements are concentrated sources of nutrients (i.e. mineral and vitamins) or other substances with a nutritional or physiological effect that are marketed in “dose” form (e.g. pills, tablets, capsules, liquids in measured doses).
</p>
                <hr>
            </div>
        </div>
                ...
                <div class="card">
                    <div class="card-body collapse in">
                        <div class="card-block">
                                ...
                                <div class="form-body">
                                    <h4 class="form-section"><i class="icon-folder4"></i> {{ todo }} Food Supplement</h4>
...
export default {
    ...
    methods: {
        ...
        deleteEquipment(equipment) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('/api/food-supplements/'+equipment.id)
                    .then((res) => {
                        this.clearFields();
                        this.getEquipments();
                        Swal.fire(
        ...
        getEquipments(){
            axios.get('/api/food-supplements')
            .then((res) => {
       ...
       dataModalOpen(){
        ...
        saveData(){
            if(this.todo == 'Add'){
                axios.post('/api/food-supplements', this.datas)
                .then((res) => {
                    this.clearFields();
                    this.getEquipments();
                    Swal.fire(
                ...
                axios.patch('/api/food-supplements/'+this.datas.id, this.datas)
                .then((res) => {
                    this.clearFields();
                    this.getEquipments();
                    Swal.fire(
